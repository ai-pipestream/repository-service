...
This document captured open questions and early Q&A during a previous iteration that focused on a chunked gRPC upload API.

### Current status
- The **current consolidated decisions** are documented in [`../../DESIGN.md`](../../DESIGN.md).
- The **current phase plan** is documented in [`phases.md`](phases.md).

### Why this is kept
It contains useful historical context around:
- chunk sizing
- idempotency semantics
- performance target uncertainty
- coordination sizing/ops questions

…but it should not be treated as the active specification.

### Next action
When we start implementing Phase 1 (PipeDocService), we should extract any still-relevant items from this file into either:
- `DESIGN.md` (if it’s a design decision), or
- a concrete implementation ticket/checklist (if it’s an engineering task).

---

(Original Q&A follows below.)
...
- Target sustained ingest throughput from intake → repository service (single node and cluster): e.g., 1–5 GB/s per node? Aggregate target across N nodes?
  - not sure what we would define.  we have a 10gb internal network with a lot of machines/ raspberry pis on nvme with 2.5gb connection - we may want to make a cluster to test?  But as long as it works at first so we can scale it.. we can assume memory-backed coordination is fast and object storage is slower, which is the point of doing this the way we are.
...
- Resource envelopes for a single repository-service pod: CPU cores, RAM, network bandwidth to plan buffers and concurrency defaults.
  - not sure
- Back-pressure behavior when Kafka/S3 slows down: Should intake start rejecting (fast-fail) or buffer up to bounded memory/coordination capacity with 429s when exceeded?
  - not sure..

## 3) Coordination Layer (Design & Limits)
- Coordination persistence: Should multipart/upload coordination live only in the repository DB (recommended), or do we also need a short-lived in-memory layer?
  - not sure
- Memory limits and eviction (if any in-memory coordination exists): What budget and TTLs for transient state?
  - not sure
- Tenancy isolation: Multi-tenant requirements (per-tenant prefixes/limits/quotas)?
  - not sure
- Reliability expectation: Is losing uncommitted transient upload state acceptable under worst-case coordinator loss, or must we guarantee no-loss via “write-through to object storage”?
  - not sure
...
- Source of truth for node metadata: DB vs. S3 object metadata? Current design suggests DB for durable metadata; confirm fields and write timing.
  - is that wise?
...
- Crash recovery: On service restart, do we re-scan DB-backed upload sessions to resume incomplete uploads automatically? Max resume window (TTL)?
...
## 9) Observability
- Metrics: Confirm must-have metrics (ingest QPS, chunk sizes, coordinator queue depth, S3 part durations, E2E commit latency, error rates, retries).
- Tracing: OpenTelemetry enabled with gRPC, S3, Kafka spans? Sampling rates?
...
- Feature gate sequence: Confirm we start with canonical SavePipeDoc, then performance test, then Kafka, then S3 multipart, then full lifecycle.
